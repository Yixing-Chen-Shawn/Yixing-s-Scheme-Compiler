...
(define (unify-lambdas ce)
  (match ce
    ...
    [`(,aef ,aes ...)
     ...
     (define listname (symbol-append (gensym 'argslist) '$ aef))
     ;;use prim cons to make a list in cps, this label is to prevent multiple definition of local value name
     ;;when it converted into llvm, you can set whatever number you want. 
     (define (h func args listname label)
       (match args
         ['() `(,func ,(symbol-append listname (string->symbol (number->string label))))]
         [`(,x0 ,xrest ...) `(let ([,(symbol-append listname (string->symbol (number->string (add1 label))))
                                    (prim cons ,x0 ,(symbol-append listname (string->symbol (number->string label))))])
                               ,(h aef xrest listname (add1 label)))]))
     `(let ([,(symbol-append listname (string->symbol (number->string 0))) '()])
        ,(h aef (reverse aes) listname 0))]))
